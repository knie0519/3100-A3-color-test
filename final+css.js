...
allFactors.forEach((factor) => {
    // Create an <option>country_name</option> element.
...
function setPlot(chosenFactor, filename, initial) {
    ...
    years.forEach((year) => {
        ...
        allFactors.forEach((factor) => {
            ...
            names.forEach((uniqueName) => {
                // Very similar to the function above ^, difference is we don't use years[0] but year to get ALL of the data!
                const dataObj = {
                    text: filename
    ...
    buttons.forEach((button) => {
        button.addEventListener("click", function (e) {
            ...
            //Plotly.relayout('data', dataEl.layout.annotations);
